'''
Sign your name: Ryan Muetzel

1.) How do you enter a single line comment in a program? Give an example.
#by putting a hashtag before the comment

2.) Enter a=2 and b=5 in the Python Console window and then all of the following. 
What are the outputs? If the output is an error record the error and try to determine what the error is!

b/a                 = 2.5  #standard division
b//a                = 2    #division but drop the remainder
b**a                = 25   #b^a
b%a                 = 1     #gives the remainder of b/a
a+B                 = 7    #literally just addition
type(42)            = 'integer'
type(42.0)          = 'float'
type("C3PO")        = 'string'
type(True)          = 'boolean'

#extra notes for myself
b=b+1               = Adds 1 to the variable 'b' , can also use b+=1
a,b =(2,5)          = Sets a=2 and b=5

3.) What is the final output of (a) and type(a) if you enter the following 5 lines
into the Python Console Window?

a=2
a*=10
a/=2
a+=12
a-=7
a             = 15
type(a)       = int


4.) What is the mistake in the following code. Fix it!

x,y = (4,5)
a = 3(x + y)  #the code doesnt know how to distribute to parenthesis
a = 3*(x+y)      # should be correct
a


5.) What is the mistake in the following code so it will calculate the average. Fix it!

x,y,z =(3,4,5)
ave = (x+y+z)/3  #added parenthesis around the addition to make it work
ave


'''
